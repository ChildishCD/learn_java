...
public class BaseDAO<T> {
        ...
        if (id == null){
            ...
            for (Field field : fields){
                ...
                if (idAnnotation == null){//不要id
                    ...
                    if (columnAnnotation!=null){
                        ...
                        Object o = getterMethod(t, field.getName());
                        if (o != null){//如果是null值，就不拼接
                            if (i!=0){
                                sql.append(",");
                                endsql.append(",");
                            }
            ...
            for (Field field : fields){
                ...
                if (idAnnotation == null){//不要id
                    ...
                    if (columnAnnotation!=null){
                        String columnName = columnAnnotation.value();
                        Object o = getterMethod(t, field.getName());//获取属性值
                        if (i!=0){
                            sql.append(",");
                        }
                        sql.append(columnName);
    ...
    //带where条件查询
    public List<T> selectListWhere(T t){
        if (t == null){
            return null;
        }
        ...
        sql.append(" where 1=1 ");//1=1永远为true，方便后面拼接sql 不用判断and
        Class clazz = getTClass();
        Field[] fields = clazz.getDeclaredFields();
        List<Object> paramList = new ArrayList<>();
        for (Field field : fields){
            ...
            if (annotation != null){// 说明有注解Column
                Object o = getterMethod(t, field.getName());
                if (o != null){
                    String clo = annotation.value();//列名 字段名
                    sql.append(" and ");//前后加空格是为了防止两个拼接的字母连一起发生错误，空格多了不会出错
                    sql.append(clo);
                    sql.append(" = ? ");
                    //获取值，拼接参数
        ...
        return selectList(sql.toString(),paramList.toArray(new Object[0]));
    }
